...
db.query("CREATE TABLE IF NOT EXISTS domains (id INTEGER PRIMARY KEY, domain TEXT NOT NULL UNIQUE, expiration INTEGER NOT NULL, nameservers TEXT, registrar TEXT NOT NULL, dns TEXT NOT NULL, clientId INTEGER NOT NULL, rawWhoisData TEXT NOT NULL, notes TEXT, FOREIGN KEY(clientId) REFERENCES clients(id))").run();
